...
export async function GET(req: Request) {
  try {
    ...
    // If SIREN (9 digits), fetch main establishment
    ...
    return NextResponse.json({
      ...
      postalCode,
      city,
      dirigeant: dirigeant ? { nom: dirigeant.nom, prenoms: dirigeant.prenoms ?? dirigeant.prenom_de_naissance ?? '' } : null,
    })
  } catch (error) {
    console.error('[GET /api/settings/verify-siren]', error)
